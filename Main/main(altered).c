...
void SaveProgress(char file_name[7],int wpm,int choice)
{
    ...
    fprintf(progress,"\t%-10s\t%-15s\t%-20s\t%-15d%-35s\n",username,mode,text,wpm,ctime(&Time));
    fclose(progress);
    ViewScores();
}
...
void ViewScores()
{
    system("cls");
    printf("\t\t\t\t\t\tS C O R E S");
    printf("\n\n\tName\t\tMode\t\tText\t\t\tScore(WPM)\t\t\tDate/Time");
...
void death_mode(char file_name[7],int choice)
{
    int i = 0, begin, end, diff, j, wrong = 0, totalcharacters = 1;
    int wpm;
    char ch, sel;
